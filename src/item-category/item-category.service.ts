...
export class ItemCategoryService {
  ...
  async findAll(query: { page?: string; limit?: string; search?: string }) {
    ...
    if (query.search && query.search.trim()) {
      ...
      const allCategories = await this.itemCategoryModel
        .find(filter)
        .sort({ createdAt: -1 })
        .exec();
    ...
    const total = await this.itemCategoryModel.countDocuments(filter);
  ...
  async create(data: { name: string; description?: string }) {
    try {
      ...
      const existing = await this.itemCategoryModel.findOne({ name: data.name });
      ...
      const newCategory = new this.itemCategoryModel({
        name: data.name,
  ...
  async update(id: string, data: { name?: string; description?: string }) {
    try {
      const category = await this.itemCategoryModel.findById(id);
      ...
      if (data.name && data.name !== category.name) {
        const existing = await this.itemCategoryModel.findOne({ 
          name: data.name,
          _id: { $ne: id },
          deleted: false 
        });
